...
def earth_periodic_term_summation(terms, count, jme):
    ...
    for i in range(count):
        sum += terms[i][TERM.A]*np.cos(terms[i][TERM.B]+terms[i][TERM.C]*jme)
    return sum

def earth_values(term_sum, count, jme):
    sum=0
    for i in range(count):
        sum += term_sum[i]*np.pow(jme, i)
    sum /= 1.0e8
    return sum
...
def nutation_longitude_and_obliquity(jce, x, del_psi, del_epsilon):
    ...
    for i in range(Y_COUNT):
        ...
        sum_psi     += (PE_TERMS[i][TERM_P.PSI_A] + jce*PE_TERMS[i][TERM_P.PSI_B])*np.sin(xy_term_sum)
        sum_epsilon += (PE_TERMS[i][TERM_P.EPS_C] + jce*PE_TERMS[i][TERM_P.EPS_D])*np.cos(xy_term_sum)
